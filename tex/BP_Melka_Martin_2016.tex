		...
		\subsection{Případy užití}
			...
			\begin{description}
				\item[UC 2.01 Odhlásit se] \hfill \\
					Odhlásí uživatele z aplikace. % TODO Má být usecase "odhlásit", když REST tohle už ze své podstaty neumožňuje?
				...
				\item[UC 2.04 Sdílet úkol] \hfill \\
					...
					% TODO sdílet by měl jenom účastník úkolu, ne jakýkliv přihlášený user.
				
				\item[UC 2.05 Zobrazit seznam mých úkolů] \hfill \\
				    1. <Filtrovat úkoly> \\
				    2. <Řadit úkoly> \\
					3. Aplikace zobrazí seznam všech úkolů, kterých je uživatel součástí, se zvoleným filtrem a řazením. \\
					
				\item[UC 2.06 Filtrovat úkoly] \hfill \\
					Úkoly je možné filtrovat podle role uživatele v nich (pozorovatel nebo pracovník), podle jejich typu (s deadlinem nebo rostoucí), stavu a priority (celé číslo 1--5).
			...
			\begin{description}
				\item[UC 3.01 Zobrazit seznam úkolů skupiny] \hfill \\
					...
					2. <Filtrovat úkoly> \\
					3. <Řadit úkoly> \\
					4. Aplikace zobrazí seznam všech úkolů skupiny se zvoleným filtrem a řazením. \\
...
\chapter{Návrh řešení}
	...
	\section{REST API}
		...
		\subsection{REST API aplikace Deadlines}
			...
			\subsubsection{\texttt{/task\{?filter\} [GET, POST]}}
	...
	\section{Návrhový model}
		% TODO popis toho, co to je návrhový model a yada yada.
		Návrhový model tříd je zachycen v diagramu \ref{diagram:class-model}.
	...
	\section{Urgentnost}
		\begin{equation}
			f(x)=\begin{cases}
				a^{x}\cdot b + c,& \text{pro } x\leq 0\\
				\lvert(a^{-x}-1)\rvert \cdot b +b+c,& \text{pro } x\geq 0
			\end{cases}
		\end{equation}
		...
		\begin{verbatim}
			...
			b = 100
			c = 0
			Plot[{ConditionalExpression[a^x*b + c, x <= 0], 
			ConditionalExpression[Abs[(a^(-x)*b) - b] + b + c, 
			x >= 0]}, {x, -40, 40}]
		...
		Experimentálně zjištěno, že nejvyšší urgence se bude pohybovat kolem ~200 (s těmito parametry).
		
		Note - s úpravou C do záporných čísel může dojít k tomu, že hodnota y bude záporná, ale to nevadí.
...
\chapter{Implementace}
	...
	http://stackoverflow.com/questions/25076718/spring-propagation-examples-in-laymans-terms/25083505\#25083505
