	...
	\section{Technologie}
		...
		\subsection{Spring}
			...
			\subsubsection{Spring Boot}
				...
				Spring Boot také umožňuje jednoduše sestavit aplikaci do podoby jednoho JAR souboru (\uv{\textit{fat jar}}), který obsahuje vše, co je nutné k jejímu spuštění. Díky vestavěnému serveru není nutné mít vlastní aplikační server, jako například Tomcat či GlassFish. Lze zvolit způsob distribuce aplikace, takže je možné ji zabalit i do WAR souboru a nasadit na existujícím serveru.
				...
				\begin{Verbatim}[obeytabs,tabsize=2]
{
	...
	\section{Autentifikace}
		Ukládání hesel uživatelů v databázi v čistém textu představuje bezpečnostní riziko. V případě, že dojde k úniku dat databáze, může útočník získat přihlašovací jména i hesla a tím i možnost přihlásit se do aplikace jako kterýkoliv uživatel. Z tohoto důvodu je vhodné neukládat samotná hesla, ale jen jejich \uv{otisk}. Ten se z hesla vytvoří pomocí \textit{hash funkce} \cite{hash-function}. Při použití vhodné funkce se jedná o relativně bezpečný způsob, kterým lze ověřovat uživatele, aniž by mohlo dojít k úniku jejich hesel.
