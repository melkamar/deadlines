...
\chapter{Analýza}
	...
	\section{Doménový model}
		Doménový model slouží k popisu dat, zachycení hlavních entit, se kterými bude aplikace pracovat, a vztahů mezi nimi. \cite{domain-model} Jedná se o abstraktní pohled na problém, a tak diagram obsahuje jen údaje dané předchozími kroky analýzy. Nejsou v něm implementační detaily, jako například konkrétní atributy a metody.
		
		Diagram doménového modelu je znázorněn na obrázku \ref{diagram:domain-model}. V této části se dále budu věnovat popisu jednotlivých entit modelu.
		
		\begin{figure}\centering
			\includegraphics[width=0.8\textwidth]{ea-diagrams/domain-model.png}
