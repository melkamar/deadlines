...
\chapter{Návrh řešení}
	...
	\section{REST API}
		...
		\subsection{REST API aplikace Deadlines}
			...
			Každý endpoint odpovídá na požadavek odpovědí, která obsahuje HTTP kód popisující, jak volání dopadlo, a případně tělo ve formátu JSON. Odpovědi od aplikace mohou mít následující kódy:
			\begin{description}
				\item[200 OK] Požadavek proběhl v pořádku.
				\item[201 CREATED] Požadovaný objekt či záznam byl vytvořen.
				\item[400 BAD REQUEST] V požadavku je chyba. Detaily o konkrétní chybě jsou obsaženy v těle zprávy, která má následující formát:
				\item[401 UNAUTHORIZED] Požadavek neobsahuje autentifikační údaje.
			...
			\end{description}
			Všechny chybové odpovědi obsahují v těle odpovědi další informace o chybě. Tělo má následující formát:
				\begin{Verbatim}[obeytabs,tabsize=2]
{
	...
	"errorMessage": "Sample error message."
			...
			\subsubsection{\texttt{/task[\{?filter\&order\}] [GET, POST]}}
			...
			\subsubsection{\texttt{/task/\{task\_id\}/reseturgency [POST]}}
			...
			\subsubsection{\texttt{/task/role/\{task\_id\}\{?newRole[\&targetUser\&targetGroup]\} [POST]}}
			...
			\subsubsection{\texttt{/group/\{group\_id\}/member/offer [POST]}}
				Zašle nabídku ke vstupu do skupiny se zadaným ID uživatelům předaným v těle požadavku. \formanagers

			\subsubsection{\texttt{/group/\{group\_id\}/member/\{member\_id\} [PUT, DELETE]}}				
			...
			\subsubsection{\texttt{/group/\{group\_id\}/task/\{task\_id\} [DELETE]}}							
...
\chapter{Implementace}
	...
	%TODO SQL create skript vzít z init logu springu - copypasta
